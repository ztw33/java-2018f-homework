...
public class UI {
    ...
    public void InitCreature(){
        ...
        demon = new Evil(battleField);
        god = new God(battleField);
        ...
        RockIcon = new ImageIcon("src/Picture/rock.jpg");
        RockIcon.setImage(RockIcon.getImage().getScaledInstance(50,50,RockIcon.getImage().SCALE_DEFAULT));
    }
    public void InitUI(){
        ...
        for(int i = 0; i < grids.length; i++) {
            for(int j = 0; j < grids[i].length; j++) {
                ...
                SetCreature(i, j, battleField.GetObject(i, j));
    ...
    public void SetCreature(int x_pos, int y_pos, Object creature){
        String ID = "Grass";
        if(creature instanceof Identity) {
            Identity a = (Identity) creature;
            ID = a.Identity();
        }
        ...
        else if(ID.equals("大石头")){
            grids[x_pos][y_pos].setIcon(RockIcon);
            return;
        }
        else{
